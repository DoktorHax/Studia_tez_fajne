...
x=-5:0.01:5; % przedzial zmiennosci x
d=get(u1,'String');
   ...
   if  a==0 
     ...
     if d>=0
        ...
        'Style','text',  'Tag','StaticText2');
         ...
         X1={'brak'};  X2={'brak'}; % brak rozwiazan rzeczywistych
...
% WYKRES paraboli
osx0=x*0;
 y=a.*x.*x+b.*x+c; plot(x,y,'-b',x,osx0,'--r');  xlabel('X');ylabel('Y'); grid on;
 hold on;
plot(X1,0,'ko',X2,0,'bo');
if (X1-X2)==0 
text(X1,1,'    X1, X2');    
else
text(X1,1,'X1'); text(X2,1,'X2');
end
plot(xw,yw,'r*');
text(xw,yw,'    xw,yw');
